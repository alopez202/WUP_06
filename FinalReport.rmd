...
Is gun ownership (OWNGUN) independent of race (RACE)?
...
```
The graph shows that among the races the black race is less likely to own a gun.
...
colPerc(table1)
```
The nemerical sumeriries shows that the white race is more likely to own a gun.
...
## bar chart of the expected % if null is true
...
```{r}
fisherXY(GSS$RACE,GSS$OWNGUN,printAll = T)
```


# Discussion and Conclusion

base on the p-values there is  no evidence of relationship between race and gun ownership.
